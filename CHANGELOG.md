...
- **桌面端 KySec 自适应**：venv 预检识别麒麟 KySec 拦截特征（failed to map segment），给出 `kyexectl -s -r` 信任标签指引，避免「探针失败→重建」死循环。
- **RPM/DEB 发布形态**：打包链改 staging 模式——`git archive` 导出干净树后应用 `desktop/packaging/release-clean.patch`（剔除手机客户端部分，仅作用于打包树，仓库保持完整）；安装包不含 venv，首启联网 pip 清华镜像拉取依赖；Release 流水线自动构建并随 GitHub Release 发布。
- 新增《使用说明书.md》（安装/配置/端点/排障，含 KySec 信任标签一节）。
