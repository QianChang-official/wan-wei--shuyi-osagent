...
- [统一文档中心](文档中心_DOCUMENTATION_HUB.md)：汇总 `docs/` 下 48 份文档，覆盖 10 个分类、42 份现行文档和 6 份历史备份，并为每份来源记录路径、状态、源提交与 SHA-256。
- `docs/` 继续保留可独立审阅的源文档；文档中心是统一导航和保真全文镜像，不替代源文件的维护历史。
- 文档中心由 `scripts/build_documentation_hub.py` 生成；运行 `py -3.11 scripts/build_documentation_hub.py --check` 可检查来源清单、锚点、全文边界、代码围栏、折叠块和生成结果是否一致。
...
| 关联检索 | Kylin 原生 embedding/vector Bridge（可选首选）+ SQLite FTS5 后备、可信检索、证据卡跳转 Capsule |
...
| 麒麟 OS Agent 适配 | [麒麟文档映射](文档中心_DOCUMENTATION_HUB.md#doc-kylin-docs-mapping-86762731)、[兼容性测试报告](文档中心_DOCUMENTATION_HUB.md#doc-compatibility-test-report-b002acd2) |
| 提交材料支撑 | 云笈导出舱、README、技术方案、测试报告、用户手册、适配测试报告映射 |

完整矩阵见 [竞赛要求覆盖矩阵](文档中心_DOCUMENTATION_HUB.md#doc-competition-requirement-coverage-beaccf0d)。

## 当前赛题就绪度

项目方向与 XA-202612「OS Agent 记忆优化及高效应用研究」高度一致，已经具备可演示的记忆写入、治理、检索、证据卡、审计、指挥闭环、复盘演化和轻量评测。但提交参赛前仍需完成 PR #21 最终合并提交的目标 VM 复验，并补齐物理目标硬件/目标架构、OCR 与量化验收材料。
...
| 待补齐 | 物理麒麟目标硬件认证、偏好提取准确率、知识检索召回率、知识冲突处理正确率、PPT 与演示视频 |

提交前 P0 建议：

- 将已完成的 Kylin V11 2603 x86_64 VM SDK 证据在指定物理目标硬件和目标架构上复验。
- 扩展标准化评测集，正式输出偏好提取准确率、知识检索召回率、冲突处理正确率，目标对齐赛题的 85% / 85% / 88%。
- 对剩余 OCR/硬件范围补充适配报告，不把 VM 结果扩大为物理硬件认证。
...
- FastAPI 后端
- SQLite + FTS5
- Kylin 原生 embedding/vector Bridge（SDK 可用时首选，FTS5 后备）
- MemoryCapsule 2.0
- Policy Gate
- Evidence Cards
- Command Loop
...
- FastAPI 挂载 `/console`
- 非 root Docker 镜像与安全默认 Compose
- Windows/Linux CI、CodeQL、依赖审查、镜像扫描与 SBOM 发布链路
- 存活/就绪探针、请求 ID 与受保护 Prometheus 指标
- SQLite 在线备份、SHA-256 清单验证与停机恢复
- 审计流水页
- Capsule 浏览器
- 搜索证据卡跳 Capsule
- 指挥闭环页
...
- [v0.8 权威技术吸收矩阵](文档中心_DOCUMENTATION_HUB.md#doc-v08-authoritative-tech-adoption-matrix-b8f18d67)
- [版本谱系](文档中心_DOCUMENTATION_HUB.md#doc-version-lineage-71608e42)
...
- 文档：[v0.9 论文系统复现层](文档中心_DOCUMENTATION_HUB.md#doc-v09-research-system-reproduction-7311053a)
...
- [v0.9.1 深做追问与视觉同步验证](文档中心_DOCUMENTATION_HUB.md#doc-v091-deep-expansion-visual-verification-9bd18cef)
- [模型网关流程](文档中心_DOCUMENTATION_HUB.md#doc-osagent-model-gateway-flow-614c6f64)
- [赛题工作流](文档中心_DOCUMENTATION_HUB.md#doc-osagent-competition-workflow-c67ae006)
...
镜像默认非 root、只读根文件系统并把数据库保存到命名卷。详细密钥、备份、监控和回滚流程见 [运维手册](文档中心_DOCUMENTATION_HUB.md#doc-operations-fb2c4fc6)。
...
完整配置、Linux 命令、本地模型接入和生产边界见 [部署说明](文档中心_DOCUMENTATION_HUB.md#doc-deployment-04615394)；项目调研与外部技术来源核验见 [项目探索与本机复现报告](文档中心_DOCUMENTATION_HUB.md#doc-project-exploration-20260710-665b6f1a)。
...
docs/                              可独立审阅的文档中心来源
文档中心_DOCUMENTATION_HUB.md      统一导航与保真全文镜像
scripts/build_documentation_hub.py 文档中心生成与确定性检查
reports/
...
- v0.10 建立的是单节点可交付工程基线，不是多副本高可用架构。
...
- PR #21 审阅前源码快照已在 Kylin V11 2603 x86_64 VM 完成 embedding/vector SDK 验收；最终合并提交仍需在目标 VM 复验，OCR Adapter、物理目标硬件和其他目标架构也仍待补齐。
...
- 公开 Release 仍由项目所有者的许可证选择阻断；自动化不会擅自替所有者决定许可证。
