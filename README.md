...
模型接入状态：通玄模型舱已接入 Windows 本地 `llama-server` 的 OpenAI-compatible endpoint，WSL 访问地址为 `http://172.29.128.1:8084/v1`；`POST /model-gateway/test` 支持 `openai_compatible` 的真实 smoke。
